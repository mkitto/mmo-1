...
namespace Managers
{
    ...
    class QuestManager: Singleton<QuestManager>
    {
        ...
        void AddNpcQuest(int npcId, Quest quest)
        {
            ...
            {
                if (quest.Define.SubmitNPC == npcId && quest.Info.Status == QuestStatus.Complete)
                {
        ...
        {
            if (quest.Info == null || quest.Info.Status == QuestStatus.Complete)  // 任务可接 或 任务完成
            {
        ...
        public void QnQuestSubmited(NQuestInfo info)
        {
            var quest = this.RefreshQuestStatus(info);
